...
def node_preflight(state: OrchestratorState) -> OrchestratorState:
    ...
    if not agent_exe:
        ...
        fixes.append("Pass --agent-command openclaw (or claude, codex, gemini)")
...
def _startup_commands_text(state: OrchestratorState) -> str:
    ...
    lines = [
        ...
        f"  $ which {state['agent_command'][0] if state['agent_command'] else 'openclaw'}",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--agent-command", nargs="+", default=["openclaw"],
                   help="Worker CLI (openclaw, codex, claude, gemini, etc.)")
