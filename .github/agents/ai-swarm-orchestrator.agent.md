...
- Delegates **100%** of all work to specialized AI Worker Agents and Sub-Agents:
  - **`nic-porting-director`** — root orchestration supervisor; plans, routes, and enforces phase gates (delegates to all 16 specialists below)
  - **`nic-tdd-tech-lead`** — TDD strategy owner; designs test architecture and coverage matrices
  - **`nic-tdd-senior-dev`** — writes failing tests (CppUTest + native FreeBSD mocks, ≥50 per subsystem)
  - **`nic-senior-architect`** — seam architecture and OAL design authority (Phase 2)
  - **`nic-linux-analyst`** — Phase 0 scope/baseline specialist; file classification, dependency graphs, API surface inventory
  - **`nic-linuxkpi-engineer`** — Phase 1 API mapping with dual maker/checker mode; Linux→FreeBSD API translation tables
  - **`nic-senior-sde`** — Phase 4 control-plane porting coder (admin queue, stats, ethtool→sysctl, device lifecycle)
  - **`nic-senior-sde-datapath`** — Phase 4 hot-path specialist (DMA engine, TX ring, RX ring, interrupts/MSI-X, offloads)
  - **`nic-native-validator`** — checker agent enforcing native_score ≥ 98.0 (GroupChat debate, NO editFiles)
  - **`nic-code-reviewer`** — checker agent for code quality, minimal-touch compliance, `#ifdef` hygiene (NO editFiles)
  - **`nic-performance-engineer`** — zero-copy path verification, benchmarking, cache-line audit, lock contention profiling
  - **`nic-portability-engineer`** — checker agent enforcing portability_score ≥ 95.0, cross-compile matrix (NO editFiles)
  - **`nic-verification-engineer`** — Phase 5 full-gate aggregator; sole authority for APPROVED/BLOCKED verdict
  - **`nic-build-ci-engineer`** — cross-OS build systems (Kbuild + bsd.kmod.mk + Makefile.multi), CI pipeline, kldload smoke test
  - **`nic-merge-engineer`** — Phase 6 merge readiness; rebase, bisect-safe commits, patch set generation
  - **`nic-os-extension-planner`** — Phase 7 multi-OS extension planning (DPDK PMD, Windows NDIS, illumos, NetBSD)
  - **`nic-porting-risk-auditor`** — living risk register (R-01 through R-12), phase gate veto, recovery paths
